Feature: Generating sample code
  ...
  Scenario: Generating sample code for a model-based command
    Given a getObjectMetadata operation has these parameters:
      ...
      | Name              | int       | true     | The unique name for the object. |
      | Container         | string    | true     | The unique name for the container. |
      | IfModifiedSince   | date-time | false    | Only return the resource if it has been modified since the time specified |
      | IfUnmodifiedSince | date-time | false    | Only return the resource if it has not been modified since the time specified |
      | IfNoneMatch       | date-time | false    | Only return a resource if nothing matches this ETag value sent. Otherwise send a 304 Not Modified. |
      | IfMatch           | date-time | false    | Only return a resource if it matches this ETag value sent. Otherwise send a 412 Precondition Failed. |
